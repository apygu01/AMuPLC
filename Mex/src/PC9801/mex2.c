    ...
    if(!su->maskflg) {
        if((k = SMF_keyin()) != -1) {
            ...
            if(k == 0x60) {        /* stop */
                ...
                posprint_mex();
                sprintf(msg,
          "ORG_Clk=%ld %d:%d:%d/%d Clk(lost)=%ld(%ld) %d:%d:%d/%d(%-8.2fsec)",
...
clockset()
{
    ...
    */
    /* real time */
    /* 1996800 / 2400 = 832 , 2457600 / 2400 = 1024 */
    if(vc) {
        k = 16640;  /* 1996800  / 120 = 16640  8 MHz */
        if(ls->clktyp == 1) k = 16657;  /* smpte flame time */
                    /* 1996800 * 1.001 / 120 = 16656.64 */
        if(ls->clktyp == 3) k = 1996800 / 3600; /* 1/3600 Sec, 1/120 flame */
        if(ls->clktyp == 4) k = 1996800 / (ls->smptef * ls->mexres);
    }
    else {
        k = 20480;    /* 2457600  / 120 = 20480 */
        if(ls->clktyp == 1) k = 20501;  /* smpte flame time */
                      /* 2457600 * 1.001 / 120 = 20500.48 */
        if(ls->clktyp == 3) k = 2457600 / 3600; /* 1/3600 Sec, 1/120 flame */
        if(ls->clktyp == 4) k = 2457600 / (ls->smptef * ls->mexres);
    }
